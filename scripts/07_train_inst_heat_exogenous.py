...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train an exogenous-only baseline for inst_heat.")
    ...
    joblib.dump(model, MODEL_DIR / f"{path.stem}_{args.target}_h{args.horizon}_exogenous.joblib")
    result.to_csv(REPORT_DIR / f"{path.stem}_{args.target}_exogenous_predictions.csv", index=False)

    md_lines = [
        "# Industrial Energy inst_heat exogenous baseline",
        "",
        f"- file: `{args.file}`",
        f"- target: `{args.target}`",
        f"- horizon: `t+{args.horizon}`",
        f"- model: `HistGradientBoostingRegressor`",
        f"- features: numeric columns only, excluding `server_time`, current target, future target label, and all target-derived features",
        "",
        "## Metrics",
        "",
        "| split | MAE | RMSE | R2 |",
    ...
    (REPORT_DIR / f"{path.stem}_{args.target}_exogenous.md").write_text("\n".join(md_lines), encoding="utf-8")
    print("\n".join(md_lines))
