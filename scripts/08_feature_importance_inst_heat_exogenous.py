...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compute feature importance for exogenous inst_heat baseline.")
    ...
    feature_cols = [
        c
        for c in df.columns
        if c not in {"server_time", args.target, "target_future"} and not c.startswith(f"{args.target}_")
    ]
    X = df[feature_cols].select_dtypes(include="number")
    y = df["target_future"]

    model_path = Path(args.model_path) if args.model_path else MODEL_DIR / f"{path.stem}_{args.target}_h{args.horizon}_exogenous.joblib"
    ...
    csv_path = REPORT_DIR / f"{path.stem}_{args.target}_exogenous_feature_importance.csv"
    md_path = REPORT_DIR / f"{path.stem}_{args.target}_exogenous_feature_importance.md"
    fig_path = FIG_DIR / f"{path.stem}_{args.target}_exogenous_feature_importance.png"

    importance.to_csv(csv_path, index=False)

    top = importance.head(20)
    lines = [
        "# Industrial Energy inst_heat exogenous feature importance",
        "",
        f"- file: `{args.file}`",
    ...
    ax.set_title(f"{path.stem} - {args.target} exogenous permutation importance")
