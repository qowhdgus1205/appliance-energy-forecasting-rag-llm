#!/usr/bin/env python3
from __future__ import annotations

import argparse
import json
import sys
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT / "src"))

from power_forecast_rag.langchain_rag import (
    LG_RAG_PROMPT,
    TfidfLGRetriever,
    build_query,
    format_documents_for_prompt,
    format_incident_evidence,
)


def load_json(path: Path):
    with path.open("r", encoding="utf-8") as f:
        return json.load(f)


def main() -> None:
    parser = argparse.ArgumentParser(description="Generate a LangChain prompt for Industrial Energy facility_a.")
    parser.add_argument(
        "--rag-context",
        default="outputs/reports/facility_a_inst_heat_rag_context.json",
        help="Compact RAG context JSON.",
    )
    parser.add_argument(
        "--index-dir",
        default="outputs/rag/lg_tfidf_index",
        help="TF-IDF index directory.",
    )
    parser.add_argument("--question", default="What should I inspect when t+1 inst_heat drops below forecast?", help="User question.")
    parser.add_argument("--language", default="Korean", help="Output language.")
    parser.add_argument("--top-k", type=int, default=4)
    args = parser.parse_args()

    context_path = ROOT / args.rag_context if not Path(args.rag_context).is_absolute() else Path(args.rag_context)
    index_dir = ROOT / args.index_dir if not Path(args.index_dir).is_absolute() else Path(args.index_dir)
    if not context_path.exists():
        raise FileNotFoundError(f"Missing context file: {context_path}")

    context = load_json(context_path)
    retriever = TfidfLGRetriever(index_dir=str(index_dir), top_k=args.top_k)
    query = build_query(context, args.question)
    docs = retriever.invoke(query)
    retrieved_context = format_documents_for_prompt(docs)
    incident_evidence = format_incident_evidence(context)
    prompt = LG_RAG_PROMPT.format(
        language=args.language,
        question=args.question,
        incident_evidence=incident_evidence,
        retrieved_context=retrieved_context,
    )

    output_dir = ROOT / "outputs" / "rag" / "langchain_prompts"
    output_dir.mkdir(parents=True, exist_ok=True)
    safe_name = context_path.stem
    prompt_path = output_dir / f"{safe_name}_prompt.md"
    prompt_path.write_text(prompt, encoding="utf-8")

    print(f"saved: {prompt_path}")
    print()
    print(prompt[:2000])


if __name__ == "__main__":
    main()

